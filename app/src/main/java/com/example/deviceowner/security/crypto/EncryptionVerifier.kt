...
class EncryptionVerifier(private val context: Context) {
    ...
    suspend fun verifyAllEncryption(): EncryptionVerificationReport = withContext(Dispatchers.IO) {
        ...
        try {
            Log.d(TAG, "🔍 Starting comprehensive encryption verification...")
            ...
            Log.d(TAG, "✅ Encryption verification complete. Status: ${if (report.isHealthy) "HEALTHY" else "UNHEALTHY"}")
    ...
    private fun verifyHardwareSecurity(): ComponentStatus {
        return try {
            ...
            ComponentStatus(
                "Hardware Security",
                isHardwareBacked,
                if (isHardwareBacked) "✅ Hardware-backed keys active" else "⚠️ Software-backed keys only",
    ...
    private fun verifyIntegritySystem(): ComponentStatus {
        return try {
            ...
            ComponentStatus(
                "HMAC Integrity",
                caught,
                if (caught) "✅ Tampering correctly detected" else "❌ Integrity check failed",
    ...
    private fun verifyPreferencesEncryption(): ComponentStatus {
        ...
        return ComponentStatus("Preferences", success, if (success) "✅ Working" else "❌ Mismatch", emptyMap())
    }
    ...
    private fun verifyDatabaseEncryption(): ComponentStatus {
        ...
        return ComponentStatus("Database", isStrong, if (isStrong) "✅ SQLCipher (High Entropy)" else "❌ Weak Passphrase", emptyMap())
    }
    ...
    private fun verifyFileEncryption(): ComponentStatus {
        ...
        file.delete()
        return ComponentStatus("File Storage", success, if (success) "✅ GCM+HKDF Working" else "❌ Mismatch", emptyMap())
    }
