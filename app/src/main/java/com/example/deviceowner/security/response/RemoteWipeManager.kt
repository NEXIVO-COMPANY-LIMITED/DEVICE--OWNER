...
class RemoteWipeManager(private val context: Context) {
    ...
    fun performRemoteWipe(reason: String = "Remote wipe requested by administrator"): Boolean {
        return try {
            ...
            Log.e(TAG, "═══════════════════════════════════════════════════════")
            Log.e(TAG, "🚨 REMOTE WIPE INITIATED")
            ...
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
                try {
                    ...
                    Log.e(TAG, "✓ Remote wipe command sent - device will reset")
                    return true
                } catch (e: Exception) {
            ...
            try {
                ...
                Log.e(TAG, "✓ Remote wipe command sent (alternative method)")
                return true
            } catch (e: Exception) {
                Log.e(TAG, "Error performing remote wipe (alternative method)", e)
            }
            
            Log.e(TAG, "❌ Failed to perform remote wipe")
            false
    ...
    fun wipeAppData(packageName: String): Boolean {
        return try {
            ...
            Log.d(TAG, "✓ App data wipe initiated for: $packageName")
            true
