        ...
        scope.launch {
            try {
                ...
                if (deviceId.isNullOrBlank() || 
                    ...
                    Log.w(TAG, "⚠️ Cannot check tampering: device not registered")
                    return@launch
                }

                Log.d(TAG, "🔍 Checking for security violations...")
                ...
                if (violations.isNotEmpty()) {
                    Log.w(TAG, "⚠️ Found ${violations.size} security violation(s)")
                    for (violation in violations) {
                        reportTampering(deviceId, violation)
                    }
                } else {
                    Log.i(TAG, "✅ No security violations detected")
                }
                
            } catch (e: Exception) {
                Log.e(TAG, "❌ Error checking tampering: ${e.message}", e)
            }
        }
    }
    ...
    private suspend fun reportTampering(deviceId: String, violation: TamperViolation) {
        try {
            ...
            if (response.isSuccessful) {
                Log.e(TAG, "🔒 TAMPERING REPORTED: ${violation.type}")
                ServerBugAndLogReporter.postLog("tamper", "Critical", "Violation: ${violation.type}")
            }
        } catch (e: Exception) {
            Log.e(TAG, "❌ Error reporting tampering: ${e.message}")
        }
    }
