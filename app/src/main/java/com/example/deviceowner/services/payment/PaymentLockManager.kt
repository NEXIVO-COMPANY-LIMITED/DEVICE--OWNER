 ...
 *
 * FLOW:
 * 1. User makes payment online → next_payment_date changes
...
class PaymentLockManager(private val context: Context) {
    ...
    fun processPaymentStatus(
        ...
        Log.d(TAG, "🔄 Processing payment status...")
        ...
        if (nextPaymentDate.isNullOrBlank()) {
            Log.d(TAG, "✅ No next payment date - device unlocked")
            handleUnlocked()
            return
        }
        ...
        if (lockState == null) {
            Log.d(TAG, "✅ No lock state determined - device unlocked")
            handleUnlocked()
            return
        }
        ...
        when (lockState.type) {
            ...
            LockScreenType.SOFT_LOCK_REMINDER -> {
                Log.i(TAG, "⏰ Payment reminder - 1 day before due")
                handleSoftLockReminder(lockState, shopName)
            }

            LockScreenType.HARD_LOCK_PAYMENT -> {
                Log.i(TAG, "🔒 Payment overdue - applying hard lock")
                handleHardLockPayment(lockState, shopName)
            }

            else -> {
                Log.d(TAG, "ℹ️ Other lock type: ${lockState.type}")
            }
        }
    }
    ...
    private fun handleUnlocked() {
        try {
            ...
            Log.e(TAG, "❌ Error handling unlock: ${e.message}", e)
        }
    }
    ...
    private fun handleSoftLockReminder(lockState: LockScreenState, shopName: String?) {
        try {
            Log.i(TAG, "📢 Showing soft lock reminder overlay")
            ...
            Log.d(TAG, "✅ Soft lock reminder shown")
        } catch (e: Exception) {
    ...
    private fun handleHardLockPayment(lockState: LockScreenState, shopName: String?) {
        try {
            ...
            Log.i(TAG, "✅ Hard lock applied - payment overdue")
        } catch (e: Exception) {
    ...
    fun verifyOfflineUnlockPassword(enteredPassword: String): Boolean {
        try {
            ...
            if (storedPassword.isNullOrBlank()) {
                Log.w(TAG, "⚠️ No offline unlock password stored")
                return false
            }
            ...
            if (isCorrect) {
                ...
                Log.w(TAG, "❌ Offline unlock password verification FAILED")
            }

            return isCorrect
        } catch (e: Exception) {
            Log.e(TAG, "❌ Error verifying offline password: ${e.message}", e)
            return false
        }
    }
    ...
    private fun getOfflineUnlockPassword(): String? {
        return try {
            ...
            Log.e(TAG, "❌ Error getting offline password: ${e.message}", e)
            null
        }
    }
    ...
    fun getCurrentPaymentLockStatus(): LockScreenState? {
        return try {
            ...
            Log.e(TAG, "❌ Error getting payment lock status: ${e.message}", e)
            null
        }
    }
    ...
    fun isPaymentHardLocked(): Boolean {
        return try {
            ...
            Log.e(TAG, "❌ Error checking payment hard lock: ${e.message}", e)
            false
        }
    }
    ...
    fun isPaymentReminderActive(): Boolean {
        return try {
            ...
            Log.e(TAG, "❌ Error checking payment reminder: ${e.message}", e)
            false
        }
    }
    ...
    fun getFormattedPaymentDueDate(): String? {
        return try {
            ...
            Log.e(TAG, "❌ Error formatting payment due date: ${e.message}", e)
            null
        }
    }
    ...
    fun getPaymentCountdown(): String? {
        return try {
            ...
            Log.e(TAG, "❌ Error getting payment countdown: ${e.message}", e)
            null
        }
    }
}
