...
object DeviceIdProvider {
    ...
    fun getDeviceId(context: Context): String? {
        ...
        try {
            ...
            if (!id.isNullOrBlank()) {
                Log.d(TAG, "🔍 Retrieved Device ID (secure): $id")
                return id
            }
        ...
        try {
            ...
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
                try {
                    ...
                    Log.i(TAG, "💾 Mirrored Device ID to Protected Encrypted Storage")
                } catch (e: Exception) {
                    Log.e(TAG, "Failed to mirror to protected storage: ${e.message}")
                }
            }
            
            Log.i(TAG, "✅ Device ID saved securely: $deviceId")
        } finally {
        ...
        try {
            ...
            if (finalId != null) {
                if (idFromCurrent == null || idFromProtected == null) {
                    Log.w(TAG, "⚠ Repairing Secure Device ID across storage boundaries...")
