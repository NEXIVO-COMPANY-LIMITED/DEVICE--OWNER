...
object HashGenerator {
    ...
    fun generateInstalledAppsHash(context: Context): String {
        return try {
            ...
            Log.d(TAG, "✅ Apps hash generated: ${hash.take(16)}...")
            hash
        } catch (e: Exception) {
            Log.e(TAG, "❌ Failed to generate apps hash: ${e.message}")
            "HASH_ERROR_APPS"
        }
    }
    ...
    fun generateSystemPropertiesHash(): String {
        return try {
            ...
            Log.d(TAG, "✅ System properties hash generated: ${hash.take(16)}...")
            hash
        } catch (e: Exception) {
            Log.e(TAG, "❌ Failed to generate system properties hash: ${e.message}")
            "HASH_ERROR_PROPS"
        }
    }
