package com.example.deviceowner.managers.protection
...
class RegistrationDataProtectionManager(private val context: Context) {
    ...
    suspend fun hasValidRegistration(): Boolean = withContext(Dispatchers.IO) {
        return@withContext try {
            ...
            Log.d(TAG, "✓ Valid protected registration found: ${registration.device_id}")
            true
        } catch (e: Exception) {
    ...
    suspend fun getStoredRegistrationData(): RegistrationData? = withContext(Dispatchers.IO) {
        return@withContext try {
            ...
            val imeiList = try {
                ...
                gson.fromJson(registration.device_imeis, type) as? List<String> ?: emptyList()
            } catch (e: Exception) {
                emptyList()
            }
            ...
            RegistrationData(
                ...
                loanId = registration.loan_number,
                imeiList = imeiList,
        ...
        try {
            ...
            if (registration != null) {
                Log.d(TAG, """
                    ...
                    - Device ID: ${registration.device_id}
