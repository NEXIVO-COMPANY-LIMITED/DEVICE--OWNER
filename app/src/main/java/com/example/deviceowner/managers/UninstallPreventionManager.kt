...
class UninstallPreventionManager(private val context: Context) {
     ...
     * Locks device after 3 removal attempts - NO BACKEND CALLS
     */
    private suspend fun handleUnauthorizedRemoval() {
        withContext(Dispatchers.IO) {
            try {
                ...
                // Lock device if 3 or more attempts - NO API CALLS
                if (attempts >= UNINSTALL_ATTEMPT_THRESHOLD) {
                    ...
                    auditLog.logIncident(
                        ...
                        severity = "CRITICAL",
                        details = "Device locked due to $attempts removal attempts"
                    )
                }
            } catch (e: Exception) {
