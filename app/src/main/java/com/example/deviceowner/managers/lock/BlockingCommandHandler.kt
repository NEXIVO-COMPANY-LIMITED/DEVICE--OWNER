...
class BlockingCommandHandler(private val context: Context) {
    ...
    fun executeCommand(command: BlockingCommand): CommandExecutionResult {
        try {
            Log.w(TAG, "Executing command: ${command.type} (ID: ${command.command_id})")
    ...
    private fun executeLockDevice(command: BlockingCommand): CommandExecutionResult {
        return try {
            ...
            if (deviceId.isNotEmpty()) {
                // Use PaymentUserLockManager to apply hard lock
                val paymentLockManager = PaymentUserLockManager(context)
                paymentLockManager.applyHardLockForTampering(deviceId, command.reason)
            }
