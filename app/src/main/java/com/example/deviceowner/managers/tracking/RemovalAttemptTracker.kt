 ...
 * Applies HARD LOCK after 3 removal attempts
 */
class RemovalAttemptTracker(private val context: Context) {
    ...
    companion object {
        ...
        private const val LOCK_THRESHOLD = 3  // Hard lock after 3 attempts
    ...
    private suspend fun lockDeviceForRemovalAttempts(attemptNumber: Int) {
        withContext(Dispatchers.IO) {
            try {
                val deviceId = getDeviceId()
                if (deviceId.isEmpty()) {
                    Log.e(TAG, "Cannot lock device: device ID not available")
                    return@withContext
                }
                
                // Apply hard lock via PaymentUserLockManager
                paymentUserLockManager.applyHardLockForUnauthorizedRemoval(deviceId, attemptNumber)
                ...
                auditLog.logIncident(
                    ...
                    severity = "CRITICAL",
                    details = "Device locked due to $attemptNumber removal attempts - COMPLIANCE_VIOLATION"
                )
                
                Log.e(TAG, "✓ Device hard locked due to excessive removal attempts")
            } catch (e: Exception) {
