    ...
    fun handleLockCommand(command: LockCommand): CommandResult {
        return try {
            ...
            val lock = DeviceLock(
                ...
                backendUnlockOnly = command.lockType == LockType.PERMANENT
            )
    ...
    fun handleLoanLock(deviceId: String): CommandResult {
        return try {
            ...
            val (lockType, message) = when (loan.loanStatus) {
                ...
                "DEFAULTED" -> {
                    LockType.PERMANENT to "Loan defaulted. Device locked for repossession. Contact support."
                }
            ...
            val lock = DeviceLock(
                ...
                message = message,
                backendUnlockOnly = lockType == LockType.PERMANENT,
                metadata = mapOf(
