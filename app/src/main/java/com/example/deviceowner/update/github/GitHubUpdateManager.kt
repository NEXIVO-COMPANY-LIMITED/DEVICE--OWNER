...
class GitHubUpdateManager(private val context: Context) {
    ...
    private fun downloadFile(downloadUrl: String, outputFile: File, totalSize: Long) {
        ...
        try {
            ...
            var size = totalSize
            if (size <= 0L) size = connection.contentLengthLong
    ...
    private fun isUpdateAvailable(currentVersion: String, latestVersion: String): Boolean {
        return try {
            ...
            for (i in 0 until length) {
                val currentPart = if (i < currentParts.size) currentParts[i].replace(Regex("[^0-9]"), "").toIntOrNull() ?: 0 else 0
                val latestPart = if (i < latestParts.size) latestParts[i].replace(Regex("[^0-9]"), "").toIntOrNull() ?: 0 else 0
                when {
    ...
    private fun updateNotificationProgress(progress: Int) {
        notificationBuilder?.let { builder ->
            builder.setProgress(100, progress, false).setContentText("Downloading... $progress%")
