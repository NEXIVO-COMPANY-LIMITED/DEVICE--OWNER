...
class PolicyComplianceActivity : Activity() {
        ...
        try {
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.S) {
                if (ACTION_ADMIN_POLICY_COMPLIANCE == action) {
                    ...
                    fileLogger.logInstallationStep("Policy Compliance", "Showing UI for user confirmation")
                    ...
                    fileLogger.i(TAG, "NOTE: Diagnostic data withdrawal message is NORMAL for Android Enterprise")
                    setContentView(R.layout.activity_policy_compliance)
                    setupContinueButton(fileLogger)
                } else {
    ...
    private fun setupContinueButton(fileLogger: FileLogger) {
        try {
            ...
            continueButton.setOnClickListener {
                ...
                Log.d(TAG, "========================================")
                fileLogger.logInstallationStep("Policy Compliance", "User confirmed - Continuing provisioning")
                fileLogger.i(TAG, "User confirmed policy compliance - Continuing provisioning")
                ...
                try {
                    ...
                    setResult(Activity.RESULT_OK)
                    resultSet = true
                    fileLogger.logInstallationStep("Policy Compliance", "SUCCESS - RESULT_OK set")
                    finish()
                } catch (e: Exception) {
                    Log.e(TAG, "Error finishing PolicyComplianceActivity", e)
                    fileLogger.e(TAG, "Error finishing PolicyComplianceActivity", e)
                    ...
                    try {
                        ...
                        fileLogger.e(TAG, "Error setting result: ${e2.message}")
                    }
                    finish()
                }
            }
