...
object DeviceIdValidator {
    ...
    fun validateAndLog(deviceId: String?, context: String = ""): Boolean {
        ...
        return when (result) {
            is ValidationResult.Valid -> {
                Log.d(TAG, "âœ… Device ID valid $context: ${deviceId?.take(8)}...")
                true
            }
            is ValidationResult.Invalid -> {
                Log.w(TAG, "âŒ Device ID invalid $context: ${result.reason}")
                false
            }
        }
    }
    ...
    fun findFirstValid(vararg deviceIds: String?): String? {
        for (deviceId in deviceIds) {
            if (isValid(deviceId)) {
                Log.d(TAG, "âœ… Found valid device ID: ${deviceId?.take(8)}...")
                return deviceId
            }
        }
        Log.w(TAG, "âŒ No valid device ID found among ${deviceIds.size} candidates")
        return null
    }
}
