package com.microspace.payo.data.local.repository

import android.util.Log
import com.microspace.payo.data.local.database.AppDatabase
...
class LocalDeviceRepository(private val database: AppDatabase) {
    ...
    suspend fun saveDeviceData(
        ...
        return try {
            Log.d(TAG, "ðŸ’¾ Saving device data locally for loan: $loanNumber")
            ...
            Log.i(TAG, "âœ… Device data saved successfully with ID: $id")
            id
        } catch (e: Exception) {
            Log.e(TAG, "âŒ Error saving device data: ${e.message}", e)
            throw e
        }
    }
    ...
    suspend fun updateServerDeviceId(localId: Int, serverDeviceId: String) {
        try {
            Log.d(TAG, "ðŸ”„ Updating server device ID for local ID: $localId")
            deviceDataDao.updateServerDeviceId(localId, serverDeviceId)
            deviceDataDao.updateSyncStatus(localId, "synced")
            Log.i(TAG, "âœ… Server device ID updated")
        } catch (e: Exception) {
            Log.e(TAG, "âŒ Error updating server device ID: ${e.message}", e)
            throw e
        }
    }
    ...
    suspend fun getDeviceDataByServerDeviceId(serverDeviceId: String): DeviceDataEntity? {
        return try {
            Log.d(TAG, "ðŸ” Retrieving device data for server ID: $serverDeviceId")
            deviceDataDao.getDeviceDataByServerDeviceId(serverDeviceId)
        } catch (e: Exception) {
            Log.e(TAG, "âŒ Error retrieving device data: ${e.message}", e)
            null
        }
    }
    ...
    suspend fun getLatestDeviceData(): DeviceDataEntity? {
        return try {
            Log.d(TAG, "ðŸ” Retrieving latest device data")
            deviceDataDao.getLatestDeviceData()
        } catch (e: Exception) {
            Log.e(TAG, "âŒ Error retrieving latest device data: ${e.message}", e)
            null
        }
    }
    ...
    suspend fun updateLockStatus(serverDeviceId: String, isLocked: Boolean, reason: String?) {
        try {
            Log.d(TAG, "ðŸ”’ Updating lock status for device: $serverDeviceId, locked=$isLocked")
            ...
            if (device != null) {
                ...
                Log.i(TAG, "âœ… Lock status updated")
            }
        } catch (e: Exception) {
            Log.e(TAG, "âŒ Error updating lock status: ${e.message}", e)
        }
    }
    ...
    suspend fun updateOnlineStatus(serverDeviceId: String, isOnline: Boolean) {
        try {
            Log.d(TAG, "ðŸŒ Updating online status for device: $serverDeviceId, online=$isOnline")
            val device = deviceDataDao.getDeviceDataByServerDeviceId(serverDeviceId)
            if (device != null) {
                deviceDataDao.updateOnlineStatus(device.id, isOnline, System.currentTimeMillis())
                Log.i(TAG, "âœ… Online status updated")
            }
        } catch (e: Exception) {
            Log.e(TAG, "âŒ Error updating online status: ${e.message}", e)
        }
    }
    ...
    suspend fun saveHeartbeat(
        ...
        return try {
            Log.d(TAG, "ðŸ’¾ Saving heartbeat locally for device: $serverDeviceId")
            
            val device = deviceDataDao.getDeviceDataByServerDeviceId(serverDeviceId)
            if (device == null) {
                Log.w(TAG, "âš ï¸ Device not found for server ID: $serverDeviceId")
                return -1
            }
            ...
            Log.i(TAG, "âœ… Heartbeat saved successfully with ID: $id")
            id
        } catch (e: Exception) {
            Log.e(TAG, "âŒ Error saving heartbeat: ${e.message}", e)
            throw e
        }
    }
    ...
    suspend fun updateHeartbeatResponse(
        ...
        try {
            Log.d(TAG, "ðŸ”„ Updating heartbeat response for ID: $heartbeatId")
            ...
            if (heartbeat != null) {
                ...
                Log.i(TAG, "âœ… Heartbeat response updated")
            }
        } catch (e: Exception) {
            Log.e(TAG, "âŒ Error updating heartbeat response: ${e.message}", e)
        }
    }
    ...
    suspend fun getLatestHeartbeat(serverDeviceId: String): HeartbeatHistoryEntity? {
        return try {
            Log.d(TAG, "ðŸ” Retrieving latest heartbeat for device: $serverDeviceId")
            val device = deviceDataDao.getDeviceDataByServerDeviceId(serverDeviceId)
            if (device != null) {
            ...
            Log.e(TAG, "âŒ Error retrieving latest heartbeat: ${e.message}", e)
            null
        }
    }
    ...
    suspend fun getHeartbeatHistory(serverDeviceId: String): List<HeartbeatHistoryEntity> {
        return try {
            Log.d(TAG, "ðŸ” Retrieving heartbeat history for device: $serverDeviceId")
            val device = deviceDataDao.getDeviceDataByServerDeviceId(serverDeviceId)
            if (device != null) {
            ...
            Log.e(TAG, "âŒ Error retrieving heartbeat history: ${e.message}", e)
            emptyList()
        }
    }
    ...
    fun getHeartbeatHistoryFlow(serverDeviceId: String): Flow<List<HeartbeatHistoryEntity>> {
        return kotlinx.coroutines.flow.flow {
            try {
                ...
                Log.e(TAG, "âŒ Error retrieving heartbeat history flow: ${e.message}", e)
                emit(emptyList())
            }
        }
    }
    ...
    suspend fun getUnsyncedHeartbeats(): List<HeartbeatHistoryEntity> {
        return try {
            Log.d(TAG, "ðŸ” Retrieving unsynced heartbeats")
            heartbeatHistoryDao.getUnsyncedHeartbeats()
        } catch (e: Exception) {
            Log.e(TAG, "âŒ Error retrieving unsynced heartbeats: ${e.message}", e)
            emptyList()
        }
    }
    ...
    suspend fun updateHeartbeatSyncStatus(heartbeatId: Int, status: String, error: String? = null) {
        try {
            Log.d(TAG, "ðŸ”„ Updating heartbeat sync status for ID: $heartbeatId, status=$status")
            heartbeatHistoryDao.updateSyncStatus(heartbeatId, status, error)
            Log.i(TAG, "âœ… Heartbeat sync status updated")
        } catch (e: Exception) {
            Log.e(TAG, "âŒ Error updating heartbeat sync status: ${e.message}", e)
        }
    }
    ...
    suspend fun getDeviceStats(): Map<String, Any> {
        return try {
            ...
            Log.e(TAG, "âŒ Error getting device stats: ${e.message}", e)
            emptyMap()
        }
    }
}
