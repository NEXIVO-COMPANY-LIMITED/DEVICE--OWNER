...
class ApiClient {
        ...
        .apply {
            ...
            try {
                ...
                hostnameVerifier { hostname, session ->
                    ...
                    if (isValidHostname) {
                        Log.d("ApiClient", "âœ… SSL hostname verified: $hostname")
                        true
                    } else {
                        Log.e("ApiClient", "âŒ SSL hostname verification failed: $hostname")
                        false
                    }
                }
                ...
                Log.d("ApiClient", "âœ… Perfect SSL security configured for Device Owner")
    ...
    suspend fun registerDevice(deviceData: DeviceRegistrationRequest): Response<DeviceRegistrationResponse> {
        Log.d("ApiClient", "ðŸ” Device Owner Registration Attempt")
        ...
        try {
            ...
            Log.d("ApiClient", "ðŸ“¤ REQUEST: loan_number=${deviceData.loanNumber}, device_type=${deviceData.deviceType}")
        } catch (e: Exception) { Log.w("ApiClient", "Could not log: ${e.message}") }
        return try {
            val response = apiService.registerDevice(deviceData)
            Log.d("ApiClient", "âœ… Registration response received: HTTP ${response.code()}")
            if (response.isSuccessful) {
                Log.d("ApiClient", "âœ… Device registered successfully")
            } else {
                Log.e("ApiClient", "âŒ Registration failed: HTTP ${response.code()}")
            }
            response
        } catch (e: Exception) {
            Log.e("ApiClient", "âŒ Registration failed with exception: ${e.message}", e)
            throw e
        }
    }
    ...
    suspend fun registerDevicePayload(payload: Map<String, Any?>): Response<DeviceRegistrationResponse> {
        Log.d("ApiClient", "ðŸ” Device Owner Registration Attempt (Payload)")
        Log.d("ApiClient", "Base URL: ${AppConfig.BASE_URL}")
        Log.d("ApiClient", "Endpoint: api/devices/mobile/register/")
        Log.d("ApiClient", "Device API Key: ${if (AppConfig.DEVICE_API_KEY.isNotEmpty()) "***" else "EMPTY"}")
        try {
            val gson = com.google.gson.Gson()
            Log.d("ApiClient", "ðŸ“¤ REQUEST (flat): loan_number=${payload["loan_number"]}, device_id=${payload["device_id"]}")
        } catch (e: Exception) { Log.w("ApiClient", "Could not log: ${e.message}") }
        return try {
    ...
    suspend fun sendHeartbeat(deviceId: String, heartbeatData: HeartbeatRequest): Response<HeartbeatResponse> {
        Log.d("ApiClient", "ðŸ” Heartbeat Attempt for device: $deviceId")
        ...
        if (deviceId.isBlank() || deviceId.equals("unknown", ignoreCase = true)) {
            Log.e("ApiClient", "âŒ Heartbeat ABORTED: deviceId is blank or unknown")
        ...
        Log.d("ApiClient", "ðŸ“‹ HEARTBEAT REQUEST DATA:")
        ...
        return try {
            ...
            if (response.isSuccessful) {
                Log.d("ApiClient", "âœ… Heartbeat SUCCESS: HTTP ${response.code()}")
            } else {
                val errorBodyStr = response.errorBody()?.string() ?: "(no body)"
                Log.e("ApiClient", "âŒ Heartbeat FAILED: HTTP ${response.code()} ${response.message()}")
            ...
            Log.e("ApiClient", "âŒ Heartbeat NETWORK ERROR: No internet / DNS failure - ${e.message}")
            throw e
        } catch (e: java.net.ConnectException) {
            Log.e("ApiClient", "âŒ Heartbeat NETWORK ERROR: Cannot connect to server - ${e.message}")
            throw e
        } catch (e: javax.net.ssl.SSLException) {
            Log.e("ApiClient", "âŒ Heartbeat SSL ERROR: Certificate/handshake failed - ${e.message}")
            throw e
        } catch (e: Exception) {
            Log.e("ApiClient", "âŒ Heartbeat failed: ${e.javaClass.simpleName} - ${e.message}", e)
            throw e
        }
    }
    ...
    suspend fun sendInstallationStatus(deviceId: String, statusData: InstallationStatusRequest): Response<InstallationStatusResponse> {
        Log.d("ApiClient", "ðŸ” Installation Status: device=$deviceId, url=api/devices/mobile/{device_id}/installation-status/")
        if (deviceId.isBlank() || deviceId.equals("unknown", ignoreCase = true)) {
            Log.e("ApiClient", "âŒ Installation status ABORTED: device_id blank or unknown")
        ...
        try {
            ...
            Log.d("ApiClient", "ðŸ“¤ REQUEST DATA:")
        ...
        return try {
            ...
            Log.d("ApiClient", "âœ… Installation status response received: HTTP ${response.code()}")
            if (response.isSuccessful) {
                Log.d("ApiClient", "âœ… Installation status sent successfully")
            } else {
                Log.e("ApiClient", "âŒ Installation status failed: HTTP ${response.code()}")
                Log.e("ApiClient", "   Error body: ${response.errorBody()?.string()}")
            }
            response
        } catch (e: Exception) {
            Log.e("ApiClient", "âŒ Installation status failed with exception: ${e.message}", e)
            throw e
        }
    }
    ...
    suspend fun postDeviceLog(request: DeviceLogRequest): Response<DeviceLogResponse> {
        return try {
            ...
            if (!response.isSuccessful) {
                Log.e("ApiClient", "âŒ Device log failed: HTTP ${response.code()} ${response.errorBody()?.string()?.take(500)}")
            }
            response
        } catch (e: Exception) {
            Log.e("ApiClient", "âŒ Post device log failed: ${e.message}", e)
            throw e
        }
    }
    ...
    suspend fun postBugReport(request: BugReportRequest): Response<BugReportResponse> {
        return try {
            ...
            Log.e("ApiClient", "âŒ Post bug report failed: ${e.message}", e)
            throw e
        }
    }

    suspend fun postTamperEvent(deviceId: String, request: TamperEventRequest): Response<TamperEventResponse> {
        Log.d("ApiClient", "ðŸ” Tamper report: device=$deviceId, type=${request.tamperType}, url=api/tamper/mobile/{device_id}/report/")
        if (deviceId.isBlank() || deviceId.equals("unknown", ignoreCase = true)) {
            Log.e("ApiClient", "âŒ Tamper ABORTED: device_id blank or unknown")
        ...
        return try {
            ...
            if (response.isSuccessful) {
                Log.d("ApiClient", "âœ… Tamper event sent: ${request.tamperType}")
            } else {
                val errBody = response.errorBody()?.string()?.take(200) ?: ""
                Log.e("ApiClient", "âŒ Tamper failed: HTTP ${response.code()} $errBody")
            }
            response
        } catch (e: Exception) {
            Log.e("ApiClient", "âŒ Post tamper event failed: ${e.message}", e)
            throw e
        }
    }
    ...
    suspend fun getDeviceInstallments(deviceId: String): Response<com.microspace.payo.data.models.payment.InstallmentsResponse> {
        return try {
            ...
            Log.e("ApiClient", "âŒ Get device installments failed: ${e.message}", e)
            throw e
        }
    }

    suspend fun confirmDeactivation(deviceId: String, status: String, message: String): Response<Map<String, Any>> {
        Log.d("ApiClient", "ðŸ” Deactivation Confirmation: device=$deviceId, status=$status")
        Log.d("ApiClient", "   URL: ${AppConfig.BASE_URL}api/devices/$deviceId/confirm-deactivation/")
        Log.d("ApiClient", "   X-Device-Api-Key: ${if (AppConfig.DEVICE_API_KEY.isNotEmpty()) "***present" else "EMPTY"}")
        
        if (deviceId.isBlank() || deviceId.equals("unknown", ignoreCase = true)) {
            Log.e("ApiClient", "âŒ Deactivation confirmation ABORTED: deviceId is blank or unknown")
        ...
        return try {
            ...
            if (response.isSuccessful) {
                Log.d("ApiClient", "âœ… Deactivation confirmation SUCCESS: HTTP ${response.code()}")
            } else {
                val errorBodyStr = response.errorBody()?.string() ?: "(no body)"
                Log.e("ApiClient", "âŒ Deactivation confirmation FAILED: HTTP ${response.code()} ${response.message()}")
                Log.e("ApiClient", "   Error body: $errorBodyStr")
            }
            response
        } catch (e: Exception) {
            Log.e("ApiClient", "âŒ Deactivation confirmation failed: ${e.javaClass.simpleName} - ${e.message}", e)
            throw e
        }
    }
}
