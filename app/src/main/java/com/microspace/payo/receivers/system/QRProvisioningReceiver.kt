    ...
    private suspend fun handleQRProvisioning(context: Context) {
        try {
            ...
            context.getSharedPreferences("device_owner_prefs", Context.MODE_PRIVATE)
                .edit().putBoolean("skip_security_restrictions", true).apply()
            Log.d(TAG, "Skip security restrictions set FIRST - keyboard and touch enabled")
            ...
            // Step 4: Setup-only restrictions and block factory reset immediately
            ...
            if (factoryResetBlocked) {
                Log.d(TAG, "Factory reset BLOCKED immediately after QR provisioning")
            } else {
                Log.e(TAG, "Factory reset block failed - check Device Owner")
            }

            // Block USB debugging and Developer Options menu at provisioning
            try {
            ...
            // IMPORTANT:
            // Do NOT launch any Activity here. On Android 10+ the system can block
            // background UI launches during provisioning, causing Setup Wizard to abort.
            // The user will land on the home screen and can open the app normally.

            // Step 6: Schedule periodic work
            schedulePeriodicWork(context)
            
            Log.d(TAG, "QR provisioning completed - periodic work scheduled")
