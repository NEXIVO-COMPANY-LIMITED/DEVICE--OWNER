        ...
        when (intent.action) {
            Intent.ACTION_PACKAGE_REMOVED -> {
                ...
                if (!isReplacing && packageName == context.packageName) {
                    Log.e(TAG, "âš ï¸ DEVICE OWNER APP UNINSTALL ATTEMPT DETECTED!")
            ...
            Intent.ACTION_PACKAGE_FULLY_REMOVED -> {
                val packageName = intent.data?.schemeSpecificPart
                if (packageName == context.packageName) {
                    Log.e(TAG, "âš ï¸ DEVICE OWNER APP FULLY REMOVED!")
            ...
            Intent.ACTION_PACKAGE_DATA_CLEARED -> {
                val packageName = intent.data?.schemeSpecificPart
                if (packageName == context.packageName) {
                    Log.e(TAG, "âš ï¸ DEVICE OWNER APP DATA CLEAR ATTEMPT DETECTED!")
    ...
    private fun reapplySecurityRestrictions(context: Context) {
        try {
            ...
            if (deviceOwnerManager.isDeviceOwner()) {
                ...
                Log.d(TAG, "âœ… Setup-only restrictions re-applied after update (keyboard stays enabled)")
            }
        } catch (e: Exception) {
