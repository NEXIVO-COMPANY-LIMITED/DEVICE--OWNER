...
class FrpHealthCheckWorker(
    ...
    override fun doWork(): Result {
        return try {
            ...
            if (result.passed) {
                Log.i(TAG, "âœ… FRP health check PASSED")
                Result.success()
            } else {
                Log.e(TAG, "âŒ FRP health check FAILED")
    ...
    override fun doWork(): Result {
        return try {
            ...
            if (gmsInfo.applicationInfo?.enabled == true) {
                Log.d(TAG, "âœ… Google Play Services available and enabled")
                Result.success()
            } else {
                Log.w(TAG, "âš ï¸ Google Play Services disabled - attempting to enable")
