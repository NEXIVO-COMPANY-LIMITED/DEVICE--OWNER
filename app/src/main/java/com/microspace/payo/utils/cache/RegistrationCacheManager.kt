...
object RegistrationCacheManager {
    ...
    suspend fun clearRegistrationCache(context: Context) {
        try {
            withContext(Dispatchers.IO) {
                ...
                Log.i(TAG, "âœ… Registration cache cleared successfully")
            }
        } catch (e: Exception) {
            Log.e(TAG, "âŒ Error clearing registration cache: ${e.message}", e)
        }
    }
    ...
    suspend fun clearRegistrationCacheForLoan(context: Context, loanNumber: String) {
        try {
            withContext(Dispatchers.IO) {
                ...
                Log.i(TAG, "âœ… Registration cache cleared for loan: $loanNumber")
            }
        } catch (e: Exception) {
            Log.e(TAG, "âŒ Error clearing cache for loan $loanNumber: ${e.message}", e)
        }
    }
    ...
    fun clearDeviceDataCache(context: Context) {
        try {
            ...
            Log.i(TAG, "âœ… Device data cache cleared")
        } catch (e: Exception) {
            Log.e(TAG, "âŒ Error clearing device data cache: ${e.message}", e)
        }
    }
    ...
    suspend fun clearAllRegistrationCaches(context: Context) {
        try {
            ...
            Log.i(TAG, "âœ… All registration caches cleared successfully")
        } catch (e: Exception) {
            Log.e(TAG, "âŒ Error clearing all caches: ${e.message}", e)
        }
    }
}
