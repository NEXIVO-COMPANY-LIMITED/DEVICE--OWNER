...
class HardLockManager(private val context: Context) {
    ...
    fun applyHardLock(
        ...
        Log.w(TAG, "ðŸ”’ Applying HARD LOCK - Source: $source, Reason: $reason")
        
        try {
            saveLockState("hard", reason, source)
            
            if (!devicePolicyManager.isAdminActive(adminComponent)) {
                Log.e(TAG, "âš ï¸ Device admin not active - showing overlay only")
                showLockOverlay(reason, recommendation)
                return
            }
            
            Log.d(TAG, "ðŸ” Calling DevicePolicyManager.lockNow()")
            devicePolicyManager.lockNow()
            
            showLockOverlay(reason, recommendation)
            
            Log.i(TAG, "âœ… Hard lock applied successfully")
    ...
    private fun showLockOverlay(reason: String, recommendation: String?) {
        Log.d(TAG, "ðŸ“± Showing lock overlay")
        ...
        try {
            ...
            Log.e(TAG, "âŒ Error showing lock overlay: ${e.message}", e)
        }
    }
