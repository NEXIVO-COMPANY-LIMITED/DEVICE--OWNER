...
class DeviceOwnerSSLManager(private val context: Context) {
    ...
    suspend fun initializeSSLSecurity(): SSLSecurityResult = withContext(Dispatchers.IO) {
        Log.d(TAG, "ðŸ”’ Initializing SSL security for Device Owner mode...")
        ...
        try {
            ...
            Log.d(TAG, "ðŸ“œ Certificate Information:")
            ...
            Log.d(TAG, "âœ… SSL security initialization completed successfully")
            
        } catch (e: Exception) {
            Log.e(TAG, "âŒ SSL security initialization failed: ${e.message}", e)
    ...
    private suspend fun testDeviceOwnerCompatibility(): Boolean = withContext(Dispatchers.IO) {
        return@withContext try {
            ...
            if (isDeviceOwner) {
                Log.d(TAG, "ðŸ” Testing Device Owner SSL compatibility...")
                ...
                Log.d(TAG, if (compatible) "âœ… Device Owner SSL compatibility: PASSED" else "âŒ Device Owner SSL compatibility: FAILED")
                compatible
            } else {
                Log.d(TAG, "â„¹ï¸ Not in Device Owner mode - SSL compatibility assumed")
                true
            }
        } catch (e: Exception) {
            Log.e(TAG, "âŒ Device Owner SSL compatibility test failed: ${e.message}")
            false
        }
    }
    ...
    private fun generateSecurityRecommendations(
        ...
        if (certInfo.leafCertificatePin.isNotEmpty()) {
            recommendations.add("âœ… Update network_security_config.xml with certificate pin: ${certInfo.leafCertificatePin}")
            recommendations.add("âœ… Update ApiClient.kt with certificate pin: sha256/${certInfo.leafCertificatePin}")
        }
        
        // TLS version recommendation
        if (sslValidation.tlsVersion.isNotEmpty()) {
            recommendations.add("âœ… TLS Version: ${sslValidation.tlsVersion} (secure)")
        }
        
        // Certificate validity
        if (sslValidation.certificateValid) {
            recommendations.add("âœ… Certificate is valid and not expired")
        } else {
            recommendations.add("âŒ Certificate validation failed - check server SSL configuration")
        }
        
        // Device Owner specific recommendations
        recommendations.add("ðŸ”’ Device Owner mode requires HTTPS - HTTP will be blocked")
        recommendations.add("ðŸ”’ Certificate pinning provides maximum security against MITM attacks")
        recommendations.add("ðŸ”’ Regular certificate monitoring recommended for production")
