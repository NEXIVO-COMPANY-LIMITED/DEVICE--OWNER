...
class EnhancedTamperDetector(private val context: Context) {
    ...
    fun startMonitoring() {
        if (isMonitoring.getAndSet(true)) {
            Log.d(TAG, "â³ Monitoring already active")
            return
        }
        
        Log.i(TAG, "ðŸš€ Starting ENHANCED tamper detection (every ${CHECK_INTERVAL_MS}ms)")
        ...
        monitoringJob = scope.launch {
            while (isActive) {
                try {
                    ...
                    Log.e(TAG, "âŒ Check error: ${e.message}")
                    delay(CHECK_INTERVAL_MS)
                }
            }
        }
    ...
    private suspend fun performTamperCheck() {
        try {
            ...
            // 1. Developer Options and USB Debugging are allowed â€“ no tamper detection
            ...
            Log.e(TAG, "âŒ Tamper check error: ${e.message}")
        }
    }
    ...
    private suspend fun processDetections(detections: List<TamperDetection>) {
        for (detection in detections) {
            Log.e(TAG, "ðŸš¨ TAMPER DETECTED: ${detection.type} (${detection.severity})")
    ...
    fun getTamperStats(): String {
        return """
            ðŸš¨ TAMPER DETECTION STATISTICS:
            â€¢ Total Detections: ${tamperCount.get()}
            â€¢ Critical Detections: ${criticalTamperCount.get()}
            â€¢ Is Monitoring: ${isMonitoring.get()}
            â€¢ Developer Options: ${isDeveloperOptionsEnabled()}
            â€¢ USB Debugging: ${isUsbDebuggingEnabled()}
            â€¢ Bootloader Unlocked: ${isBootloaderUnlocked()}
            â€¢ Device Rooted: ${checkForRooting()}
        """.trimIndent()
    }
    ...
    fun stopMonitoring() {
        try {
            Log.d(TAG, "ðŸ›‘ Stopping tamper detection")
            isMonitoring.set(false)
            monitoringJob?.cancel()
            scope.cancel()
            Log.i(TAG, "âœ… Tamper detection stopped")
        } catch (e: Exception) {
