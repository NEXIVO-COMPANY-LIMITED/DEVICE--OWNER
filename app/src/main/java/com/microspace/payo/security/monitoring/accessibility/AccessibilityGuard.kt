...
class AccessibilityGuard(private val context: Context) {
    ...
    fun startMonitoring() {
        ...
        isMonitoring = true
        Log.d(TAG, "ðŸ›¡ï¸ Starting Accessibility Guard Monitoring")
    ...
    private suspend fun checkAccessibilityServices() {
        ...
        if (newServices.isNotEmpty()) {
            Log.w(TAG, "âš ï¸ New accessibility services detected: $newServices")
            
            // Check each new service
            newServices.forEach { serviceId ->
                if (!isAllowedService(serviceId)) {
                    Log.e(TAG, "ðŸš¨ UNAUTHORIZED ACCESSIBILITY SERVICE DETECTED: $serviceId")
                    handleUnauthorizedAccessibilityService(serviceId)
                } else {
                    Log.d(TAG, "âœ“ Allowed accessibility service: $serviceId")
                }
            }
        }
        ...
        if (unauthorizedServices.isNotEmpty()) {
            Log.e(TAG, "ðŸš¨ UNAUTHORIZED ACCESSIBILITY SERVICES ACTIVE: $unauthorizedServices")
    ...
    private suspend fun handleUnauthorizedAccessibilityService(serviceId: String) {
        ...
        Log.e(TAG, "ðŸš¨ UNAUTHORIZED ACCESSIBILITY SERVICE DETECTED")
    ...
    private suspend fun disableAccessibilityService(serviceId: String) {
        try {
            ...
            try {
                ...
                val adminComponent = android.content.ComponentName(context, com.microspace.payo.receivers.admin.AdminReceiver::class.java)
