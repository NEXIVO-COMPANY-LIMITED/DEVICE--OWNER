...
class DeviceOwnerCompatibilityChecker(private val context: Context) {
    ...
    data class CompatibilityResult(
        ...
        val details: String
            get() {
                val status = if (isCompatible) "âœ“ COMPATIBLE" else "âŒ NOT COMPATIBLE"
                val deviceDetails = """
    ...
    fun checkCompatibility(): CompatibilityResult {
        ...
        if (!isGooglePixel(deviceInfo.brand, deviceInfo.model, deviceInfo.manufacturer)) {
            ...
            Log.w(TAG, "âŒ Device brand check failed: Not a Google Pixel")
        } else {
            Log.d(TAG, "âœ“ Device is Google Pixel")
        }
        ...
        if (deviceInfo.androidVersion < 13) {
            ...
            Log.w(TAG, "âŒ Android version check failed: ${deviceInfo.androidVersion}")
        } else {
            Log.d(TAG, "âœ“ Android version check passed: ${deviceInfo.androidVersion}")
        }
        ...
        if (!deviceInfo.isStockAndroid) {
            ...
            Log.w(TAG, "âŒ Stock Android check failed")
        } else {
            Log.d(TAG, "âœ“ Stock Android check passed")
        }
        */
        // ===== END DISABLED CHECKS =====
        ...
        Log.d(TAG, "=== Compatibility Result: FORCED COMPATIBLE âœ“ ===")
