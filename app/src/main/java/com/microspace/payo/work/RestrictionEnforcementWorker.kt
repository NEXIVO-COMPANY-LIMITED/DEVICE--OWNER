...
class RestrictionEnforcementWorker(
    ...
    override suspend fun doWork(): Result = withContext(Dispatchers.IO) {
        return@withContext try {
            ...
            Log.d(TAG, "Periodic setup-only + silent verification â€“ keyboard/touch stay enabled until hard lock")
            Result.success()
        } catch (e: Exception) {
    ...
    companion object {
        ...
        fun schedule(context: Context) {
            ...
            Log.d(TAG, "âœ… Scheduled periodic restriction enforcement")
        }
    }
}
