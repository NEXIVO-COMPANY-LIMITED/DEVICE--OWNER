...
class EnhancedOfflineSyncService(
    ...
    fun startMonitoring() {
        try {
            Log.i(TAG, "ðŸš€ Starting ENHANCED offline/online sync monitoring")
            
            networkCallback = object : ConnectivityManager.NetworkCallback() {
                override fun onAvailable(network: Network) {
                    super.onAvailable(network)
                    Log.i(TAG, "âœ… NETWORK AVAILABLE - Triggering immediate sync")
                    triggerImmediateSync()
                }
                
                override fun onLost(network: Network) {
                    super.onLost(network)
                    Log.w(TAG, "âš ï¸ NETWORK LOST - Will queue events locally")
                }
                ...
                override fun onCapabilitiesChanged(network: Network, capabilities: NetworkCapabilities) {
                    ...
                    Log.d(TAG, "ðŸ”„ Network capabilities changed - Internet: $hasInternet, Validated: $hasValidated")
                    
                    if (hasInternet && hasValidated) {
                        Log.i(TAG, "âœ… VALIDATED INTERNET - Triggering sync")
                        triggerImmediateSync()
                    }
                }
            }
            ...
            Log.i(TAG, "âœ… Network monitoring registered")
            ...
            Log.e(TAG, "âŒ Error starting monitoring: ${e.message}", e)
        }
    }
    ...
    private fun triggerImmediateSync() {
        if (isSyncing.getAndSet(true)) {
            Log.d(TAG, "â³ Sync already in progress - skipping")
            return
        }
        ...
        scope.launch {
            try {
                ...
                Log.e(TAG, "âŒ Sync timeout after ${SYNC_TIMEOUT_MS}ms")
                isSyncing.set(false)
            } catch (e: Exception) {
                Log.e(TAG, "âŒ Sync error: ${e.message}", e)
                isSyncing.set(false)
            }
        }
    }
    ...
    private suspend fun performSync() {
        try {
            ...
            if (events.isEmpty()) {
                Log.d(TAG, "âœ… No events to sync")
                isSyncing.set(false)
                return
            }
            ...
            Log.i(TAG, "ðŸ“Š Starting sync of ${events.size} events")
            
            // Process in batches
            events.chunked(BATCH_SIZE).forEachIndexed { batchIndex, batch ->
                Log.d(TAG, "ðŸ“¦ Processing batch ${batchIndex + 1}/${(events.size + BATCH_SIZE - 1) / BATCH_SIZE}")
            ...
            Log.i(TAG, "âœ… Sync complete - Synced: ${syncedCount.get()}, Failed: ${failedCount.get()}, Total: ${totalEvents.get()}")
        ...
        try {
            val success = when (event.eventType) {
                "HEARTBEAT" -> syncHeartbeatEvent(event)
                    ...
                    Log.w(TAG, "âš ï¸ Unknown event type: ${event.eventType}")
            ...
            if (success) {
                ...
                Log.d(TAG, "âœ… Event ${event.id} synced successfully")
            } else {
                if (retryCount < MAX_RETRIES) {
                    val delayMs = calculateBackoffDelay(retryCount)
                    Log.w(TAG, "â³ Retrying event ${event.id} in ${delayMs}ms (attempt ${retryCount + 1}/$MAX_RETRIES)")
                    delay(delayMs)
                    syncEventWithRetry(event, retryCount + 1)
                } else {
                    failedCount.incrementAndGet()
                    Log.e(TAG, "âŒ Event ${event.id} failed after $MAX_RETRIES retries")
                }
            }
        } catch (e: Exception) {
            if (retryCount < MAX_RETRIES) {
                val delayMs = calculateBackoffDelay(retryCount)
                Log.w(TAG, "â³ Retrying event ${event.id} after exception in ${delayMs}ms")
                delay(delayMs)
                syncEventWithRetry(event, retryCount + 1)
            } else {
                failedCount.incrementAndGet()
                Log.e(TAG, "âŒ Event ${event.id} failed: ${e.message}")
            }
        }
    }
    ...
    private suspend fun syncHeartbeatEvent(event: OfflineEvent): Boolean {
        return try {
            ...
            if (deviceId.isNullOrBlank()) {
                Log.w(TAG, "âš ï¸ Device ID missing - keeping event for later")
                return false
            }
            
            Log.d(TAG, "ðŸ“¤ Syncing heartbeat for device: $deviceId")
            
            val response = apiClient.sendHeartbeat(deviceId, heartbeatRequest)
            
            if (response.isSuccessful) {
                Log.i(TAG, "âœ… Heartbeat synced successfully")
                true
            } else {
                Log.e(TAG, "âŒ Heartbeat sync failed: HTTP ${response.code()}")
                false
            }
        } catch (e: Exception) {
            Log.e(TAG, "âŒ Error syncing heartbeat: ${e.message}")
            false
        }
    }
        ...
        cleanupJob = scope.launch {
            while (isActive) {
                try {
                    ...
                    Log.e(TAG, "âŒ Cleanup error: ${e.message}")
                }
            }
        }
    }
    ...
    private suspend fun performCleanup() {
        try {
            ...
            if (oldEvents.isNotEmpty()) {
                ...
                Log.i(TAG, "ðŸ—‘ï¸ Cleaned up ${oldEvents.size} old events")
            }
        } catch (e: Exception) {
            Log.e(TAG, "âŒ Cleanup failed: ${e.message}")
        }
    }
    ...
    fun isOnline(): Boolean {
        return try {
            ...
            Log.e(TAG, "âŒ Error checking online status: ${e.message}")
            false
        }
    }
    ...
    suspend fun queueEvent(eventType: String, jsonData: String) {
        try {
            ...
            Log.d(TAG, "ðŸ’¾ Event queued: $eventType")
        } catch (e: Exception) {
            Log.e(TAG, "âŒ Error queuing event: ${e.message}")
        }
    }
    ...
    fun getSyncStats(): String {
        return """
            ...
            â€¢ Total Events: ${totalEvents.get()}
            â€¢ Synced: ${syncedCount.get()}
            â€¢ Failed: ${failedCount.get()}
            â€¢ Success Rate: ${if (totalEvents.get() > 0) (syncedCount.get() * 100 / totalEvents.get()) else 0}%
            â€¢ Is Syncing: ${isSyncing.get()}
            â€¢ Is Online: ${isOnline()}
        """.trimIndent()
    }
    ...
    fun stopMonitoring() {
        try {
            Log.d(TAG, "ðŸ›‘ Stopping enhanced sync monitoring")
            ...
            Log.i(TAG, "âœ… Enhanced sync monitoring stopped")
        } catch (e: Exception) {
            Log.e(TAG, "âŒ Error stopping monitoring: ${e.message}")
        }
    }
}
