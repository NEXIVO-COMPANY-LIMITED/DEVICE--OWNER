...
class TamperDetectedActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        try {
            ...
            Log.d(TAG, "â•‘ ðŸ”´ TAMPER DETECTED ACTIVITY CREATED")
            ...
            Log.d(TAG, "ðŸ“± Screen: Tamper Detected")
            Log.d(TAG, "ðŸ”’ Unlock: NOT POSSIBLE (permanent lock)")
            
            setContent {
                DeviceOwnerTheme {
                    // Placeholder - use the lock activity version instead
                    com.microspace.payo.ui.activities.lock.security.HardLockSecurityViolationScreen(
                        reason = reason,
                        lockedAt = timestamp,
                        onOpenSettings = null
                    )
            ...
            Log.e(TAG, "âŒ Error in onCreate: ${e.message}", e)
    ...
    private fun handleContactSupport() {
        try {
            Log.d(TAG, "ðŸ“ž Contact Support clicked")
            recordSuccess("CONTACT_SUPPORT_CLICKED", "User requested support")
        } catch (e: Exception) {
            Log.e(TAG, "âŒ Error in handleContactSupport: ${e.message}")
    ...
    override fun onDestroy() {
        try {
            ...
            Log.e(TAG, "âŒ Error in onDestroy: ${e.message}")
        }
        super.onDestroy()
    }
    ...
    private fun recordSuccess(action: String, details: String) {
        try {
            ...
            Log.d(TAG, "ðŸ“ Audit: $action - $details")
        } catch (e: Exception) {
    ...
    private fun recordError(action: String, error: String) {
        try {
            ...
            Log.e(TAG, "ðŸ“ Audit Error: $action - $error")
        } catch (e: Exception) {
