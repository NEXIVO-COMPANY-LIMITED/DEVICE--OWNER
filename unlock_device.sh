...
adb shell "am start -n com.example.deviceowner/.presentation.activities.MainActivity" 2>/dev/null
echo "    ✓ Done"
...
APP_PID=$(adb shell "pidof com.example.deviceowner" 2>/dev/null)
if [ -n "$APP_PID" ]; then
    echo "  ⚠ App still running - attempting harder reset..."
    adb shell "su -c 'pm clear com.example.deviceowner'" 2>/dev/null
    adb shell "su -c 'am force-stop com.example.deviceowner'" 2>/dev/null
else
