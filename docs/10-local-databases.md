...
**Class**: `com.example.deviceowner.data.local.database.DeviceOwnerDatabase`  
...
- **DeviceOwnerDatabase.getDatabase(context)** — singleton instance.  
...
- **device_registration** — device_id, loan number  
- **device_data** — device_id_for_heartbeat  
- **device_owner_config** — security mode, etc.  
- **control_prefs** — lock state, reason, timestamp (and device-protected copy for boot)  
- **device_locks** (HardLockManager) — lock_type, lock_reason, lock_timestamp, lock_source  

These are the “live” settings; Room is used for queue and history.
...
Related: [Device Heartbeat](07-device-heartbeat.md), [Device Tamper](09-device-tamper.md), [Hard Lock and Soft Lock](08-hard-lock-and-soft-lock.md).
