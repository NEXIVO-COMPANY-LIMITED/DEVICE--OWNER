...
- **Tamper**: Critical tamper (bootloader unlock, developer options, USB debug, root, SIM change, Device Owner removal, package removal) → **EnhancedAntiTamperResponse** applies hard lock immediately, then notifies server.
- **Boot**: **TamperBootChecker** detects violation on boot → hard lock + tamper report.
...
- **Payment reminder**: Heartbeat returns e.g. `next_payment` (e.g. 1 day before due) → app shows a **reminder overlay** so the user can pay in time.  
- **SIM change**: **SIMChangeDetector** detects SIM change → show **SIMChangeOverlayActivity** or **SoftLockOverlayService** with SIM warning; user taps “Continue” to dismiss.  
...
1. **SoftLockOverlayService**  
   ...
   - **startOverlay(context, reason, triggerAction, nextPaymentDate, organizationName)** — starts overlay with reason and optional next payment date / org name.  
   - **stopOverlay(context)** — dismisses overlay.  
   - User taps “I UNDERSTAND” / “CONTINUE” → overlay is closed; device remains usable (no kiosk).
...
3. **Soft lock types** (`SoftLockType`)  
   - **PAYMENT_REMINDER**: Payment due soon; button “I UNDERSTAND”.  
   - **SIM_CHANGE**: Unauthorized SIM detected; button “CONTINUE”. Event is logged; user can continue using the device after acknowledging.
...
- Hard lock persists across reboot; soft lock is session-only (no persistent “soft locked” state after reboot).
...
Related: [Device Tamper](09-device-tamper.md), [Device Heartbeat](07-device-heartbeat.md), [Services](13-services.md).
