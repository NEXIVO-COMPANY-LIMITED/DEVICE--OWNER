...
2. **Responds locally** by applying **hard lock** (kiosk) immediatelyâ€”without waiting for the server.  
...
- **Response**: If any violation â†’ **RemoteDeviceControlManager.applyHardLock()** with `tamperType`; **EnhancedAntiTamperResponse.sendTamperToBackendOnly()** sends the tamper event to the server.
...
- If a violation is detected â†’ soft lock or escalation to hard lock and tamper report, depending on implementation (e.g. **DeviceViolationDetector**, **EnhancedTamperDetector**).

### 3. Device Owner removal (DeviceOwnerRemovalDetector)

- **DeviceOwnerRemovalDetector** (part of the â€œSecurity Meshâ€ in SecurityMonitorService) checks periodically (e.g. every 5 seconds) whether the app is still Device Owner.  
- If Device Owner was removed â†’ hard lock + remote wipe and tamper report (e.g. SYSTEM_MODIFIED / DEVICE_OWNER_REMOVED).

### 4. SIM change (SIMChangeDetector)

- **SIMChangeDetector** monitors SIM state. On change â†’ **SIMChangeOverlayActivity** (warning screen) and tamper report with type **SIM_CHANGE**.
...
- If the Device Owner app (or critical component) is removed or data cleared â†’ tamper type **PACKAGE_REMOVED** and appropriate response (hard lock / report).

### 6. Firmware / integrity (FirmwareSecurityMonitorService, EnhancedTamperDetector)

- **FirmwareSecurityMonitorService** and **EnhancedTamperDetector** (or similar) detect root, custom ROM, system modifications â†’ tamper type ROOT_DETECTED, CUSTOM_ROM, SYSTEM_MODIFIED, etc.
...
2. **respondWithHardLockAndNotify**  
   ...
   - **Step 2**: In background, **notifyRemoteServer()** â€” build **TamperEventRequest.forDjango()**, call **ApiClient** to POST to **api/tamper/mobile/{device_id}/report/**. On failure or offline, persist to **OfflineEvent** for later sync.
...
- **Request**: **TamperEventRequest** â€” `tamper_type`, `severity` (CRITICAL), `description`, `timestamp` (ISO-8601), `extra_data`.  
- **Response**: **TamperEventResponse** â€” `locked` (boolean).  
...
For **logs** and **bugs** (which can include tamper-related messages), see [11.0 Device Logs and Bugs](11.0-DEVICE-LOGS-AND-BUGS.md).
