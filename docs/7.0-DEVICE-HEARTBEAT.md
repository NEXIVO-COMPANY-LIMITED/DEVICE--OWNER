...
**⚠️ If keys don't match → 403 Forbidden error → Heartbeat fails**
...
```
IF IMEI changed → is_locked = true
IF Device rooted → is_locked = true
IF RAM decreased significantly → is_locked = true
IF SIM changed → is_locked = true
```
