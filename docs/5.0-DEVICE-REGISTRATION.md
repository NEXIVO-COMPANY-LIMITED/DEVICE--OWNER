...
The app resolves "current device ID" from these sources (e.g. in **HeartbeatWorker**, **ServerBugAndLogReporter**, **EnhancedAntiTamperResponse**): typically `device_data.device_id_for_heartbeat` â†’ `device_registration.device_id` â†’ **SharedPreferencesManager.getDeviceIdForHeartbeat()**.

---

## Registration errors

- **RegistrationErrorViewerActivity** â€” Lists and opens saved registration error files (e.g. HTML) from a dedicated folder (e.g. `DeviceOwner/RegistrationErrors`) so support can inspect failed registration attempts.
...
For **heartbeat** (which uses `device_id`), see [7.0 Device Heartbeat](7.0-DEVICE-HEARTBEAT.md).
