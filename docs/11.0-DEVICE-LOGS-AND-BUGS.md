...
**Class**: `com.microspace.payo.utils.logging.LogManager`
...
- **setOnRemoteLogCallback(callback)** â€” Set from **DeviceOwnerApplication** after **LogManager.initialize()**.  
...
**Class**: `com.microspace.payo.services.reporting.ServerBugAndLogReporter`

### Initialization

- **init(context)** â€” Call from **Application.onCreate()** so the reporter can resolve the device ID. If not called, posting still works but uses a fallback device id (e.g. `UNREGISTERED-{model}`).
...
**Category mapping (app â†’ backend LogType)**:

- DEVICE_REGISTRATION â†’ `registration`  
- HEARTBEAT â†’ `heartbeat`  
- API_CALLS â†’ `system`  
- SECURITY â†’ `security`  
- PROVISIONING â†’ `installation`  
- SYNC â†’ `data_sync`  
- ERRORS / GENERAL â†’ `error`  
- DEVICE_OWNER / DEVICE_INFO / JSON_LOGS â†’ `system`  
- Else â†’ `other`

**Level mapping**: ERROR â†’ `Error`, WARN/WARNING â†’ `Warning`, else â†’ `Normal`.
...
- **postException(throwable, contextMessage?)**  
  - Serializes the exception (stack trace, message) and calls **postBug()** with a suitable title and message (e.g. â€œUncaught exception in {thread}â€).  
