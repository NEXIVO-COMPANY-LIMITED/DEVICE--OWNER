...
- **TamperBootChecker**: On boot, check developer options, USB debugging, bootloader â†’ hard lock + tamper report.  
- **SecurityMonitorService**: Runtime checks every 2s (developer options, USB, tamper) and 500ms (Settings in foreground).  
- **DeviceOwnerRemovalDetector**: Every 5s check Device Owner status â†’ hard lock + remote wipe if removed.  
- **AccessibilityGuard**: Unauthorized accessibility services â†’ soft lock.  
- **SIMChangeDetector**: SIM change â†’ overlay + tamper report (SIM_CHANGE).  
- **PackageRemovalReceiver**: Package removed/replaced â†’ tamper (PACKAGE_REMOVED).  
- **EnhancedAntiTamperResponse**: Central handler â€” hard lock first, then POST tamper to server (or queue offline).
