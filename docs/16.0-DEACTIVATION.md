...
| **Trigger** | **Server** (heartbeat response: `deactivate_requested` / `deactivation.status == "requested"`) or **user** (MainActivity â€œREMOVE DEVICE OWNERâ€ button). |
...
- **MainActivity** (ui.activities.main) shows a â€œREMOVE DEVICE OWNER (NORMAL STATE)â€ button when the app is Device Owner.
...
| **6** | **Remove Device Owner and Admin**: **clearDeviceOwnerApp()** first (while still Device Owner), then **removeActiveAdmin()**. Order is critical so the device no longer â€œbelongs to organizationâ€. |
...
| Class | Role |
|-------|------|
| **DeviceOwnerDeactivationManager** | Runs the full deactivation sequence (phases 0â€“9). Entry: **deactivateDeviceOwner()** (suspend function). Returns **DeactivationResult.Success** or **DeactivationResult.Failure(error)**. |
...
- **LockScreenManager** / **LockScreenStrategy**: when the server sends a â€œdeactivationâ€ lock type, the UI can show a â€œdeactivation in progressâ€ or similar state; the actual removal of Device Owner is done by **DeviceOwnerDeactivationManager**.

---

## User-visible behavior

- **Before**: Device is managed (restrictions, possible lock screen, â€œManaged by organizationâ€).
