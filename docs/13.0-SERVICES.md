...
| Attribute | Value |
|-----------|--------|
| **Class** | `com.microspace.payo.monitoring.SecurityMonitorService` |
...
- **Real-Time Security Mesh**:
  - **DeviceOwnerRemovalDetector**: Every 5 seconds verifies the app is still Device Owner; on removal â†’ hard lock + remote wipe.
  - **AccessibilityGuard**: Monitors unauthorized accessibility services; triggers soft lock when detected.

**Start**

- `SecurityMonitorService.startService(context)` â€” called from `DeviceOwnerApplication` when a device ID exists (after registration).
...
| Attribute | Value |
|-----------|--------|
| **Class** | `com.microspace.payo.services.RemoteManagementService` |
| **Type** | Foreground (dataSync) |
| **directBootAware** | Yes |
...
| Attribute | Value |
|-----------|--------|
| **Class** | `com.microspace.payo.services.SoftLockOverlayService` |
| **Type** | Foreground (dataSync) |

**Purpose**

- Shows a **soft lock** overlay (full-screen or overlay window) when the server or local policy requires a â€œsoftâ€ lock (e.g. warning screen without full kiosk).
...
| Attribute | Value |
|-----------|--------|
| **Class** | `com.microspace.payo.services.SoftLockMonitorService` |
| **Type** | Foreground (dataSync) |
...
| Attribute | Value |
|-----------|--------|
| **Class** | `com.microspace.payo.services.FirmwareSecurityMonitorService` |
| **Type** | Foreground (dataSync) |
...
| Attribute | Value |
|-----------|--------|
| **Class** | `com.microspace.payo.services.LocalDataServerService` |
| **Type** | Foreground (dataSync) |
| **directBootAware** | Yes |
