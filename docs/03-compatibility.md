...
**Class**: `com.example.deviceowner.device.DeviceOwnerCompatibilityChecker`

### What is checked

1. **Android version**  
   - Must be **Android 13 or higher** (API 33+).  
   - If lower, compatibility fails with a message like: “Android version must be 13 or higher (current: X).”
...
2. **Stock Android / supported manufacturer**  
   ...
   - If not stock and not supported, issue: “Device must run stock Android or be from a supported manufacturer.”
...
- If `issues` is empty → compatible; otherwise not compatible.  
...
- **ProvisioningProgressActivity**: during initialization, calls `DeviceOwnerCompatibilityChecker.checkCompatibility()`.  
  - If compatible → navigate to **CompatibilitySuccessActivity** / **DeviceCompatibilitySuccessActivity**.  
  - If not → navigate to **CompatibilityFailureActivity** with the result (and issues).  
...
Related: [Device Installation](04-device-installation.md), [Device Owner Overview](01-device-owner-overview.md).
